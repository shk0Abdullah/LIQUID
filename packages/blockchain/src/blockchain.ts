...
export class Blockchain {
  ...
  public getMaxTransactionsPerBlock(): number {
  ...
  public async minePendingTransactions(minerAddress: string): Promise<Block> {
    ...
    const rewardTransaction = createTransaction(
      "SYSTEM",
      minerAddress,
      this.config.miningReward,
    );
    const maxPendingTransactions = Math.max(
      0,
      this.maxTransactionsPerBlock - 1,
    );
    const selectedPendingTransactions = this.pendingTransactions.slice(
      0,
      maxPendingTransactions,
    );
    const blockTransactions = [
      rewardTransaction,
      ...selectedPendingTransactions,
    ];
  ...
  public async isChainValid(): Promise<ChainValidationResult> {
    ...
    if (
      ...
      genesisBlock.previousHash !== GENESIS_BLOCK_PREVIOUS_HASH
    ) {
      return {
        isValid: false,
        invalidIndex: 0,
