  ...
  return (
    <div className="group overflow-hidden rounded-xl border border-border bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="border-b border-border bg-muted/20 px-4 py-2.5">
        ...
        <p
          className="mt-0.5 font-mono text-xs text-muted-foreground/80"
          title={node.latestHash}
        >
          {shortHash}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-y divide-border">
        ...
        <NodeStat label="Blocks" value={node.metrics.blocksAccepted} />
        <NodeStat label="Txs" value={node.metrics.txsConfirmed} />
      </div>

      {node.metrics.forksDetected > 0 && (
        <div className="border-t border-red-200 bg-red-50 px-4 py-2">
          <span className="text-2xs font-semibold text-red-600">
            {node.metrics.forksDetected} fork
            {node.metrics.forksDetected !== 1 ? "s" : ""} detected
          </span>
        </div>
      )}
      ...
      {node.neighbors.length > 0 && (
        <div className="border-t border-border px-4 py-3">
          <span className="text-2xs font-medium uppercase tracking-wider text-muted-foreground">
            Connected to {node.neighborCount}{" "}
            {node.neighborCount === 1 ? "peer" : "peers"}
          </span>
